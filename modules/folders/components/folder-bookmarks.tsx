...
export function FolderBookmarks({
  ...
  return (
    <div className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
      <div className="flex items-center space-x-2 px-6 py-3 overflow-x-auto lg:overflow-x-visible">
        <div className="flex items-center space-x-2 min-w-max lg:min-w-0">
          {/* All Notes Bookmark */}
          <motion.button
            onClick={() => onFolderSelect(null)}
            className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${
          ...
          {folders.slice(0, 5).map((folder) => (
            <div key={folder.id} className="group relative">
              <motion.button
                ...
                className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all group-hover:pr-8 whitespace-nowrap ${
          ...
          {folders.length > 5 && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <motion.button
                  className="flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all whitespace-nowrap"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
          ...
          <motion.button
            ...
            className="flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500 transition-all whitespace-nowrap"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
